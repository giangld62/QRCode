...
class ScanResultAdapter(private val viewBinderHelper: ViewBinderHelper) :
    ...
    inner class ViewHolder(val binding: ItemScanResultBinding) :
        ...
        fun bind(scanResult: ScanResult) {
            ...
            binding.ivFavorite.setOnClickListener {
                if (checkTime(it)) {
                    inter?.onFavoriteClick(
                        scanResult.id,
                        scanResult.isFavorite,
                        scanResult.rawValue!!
                    )
                    scanList[adapterPosition].isFavorite =
                        !scanList[adapterPosition].isFavorite
                    notifyItemChanged(adapterPosition, scanList[adapterPosition])
                }
            }
